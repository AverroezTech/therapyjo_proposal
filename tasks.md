...
| TJ-005 | Move content gating to a role capability | SPLIT — superseded by TJ-005a and TJ-005b; nothing executes against this ID | — |
...
| TJ-005b2 | Grant and revoke the flag from the admin UI | SPLIT — passed 2026-08-21; see TJ-005b2a, TJ-005b2b | — |
| TJ-005b2a | Grant and revoke the flag from the employee edit screens | DONE — task commit `e32d546`, merged to `master` as `5d44d91` with `--no-ff`; runtime-proven and visually reviewed | `feat/content-grant-toggle` |
| TJ-005b2b | The nav treatment, and a way in for a content-only holder | BACKLOG — no planning pass | — |
...
| TJ-011c | Patients — hard delete | CLOSED — no change needed; the user's decision of 2026-08-21 keeps the current behaviour | — |
| TJ-012 | Blog — hard delete a post | DONE — task commit `d5805fd`, merged to `master` as `fb94e81` with `--no-ff`; runtime-proven and visually reviewed | `feat/blog-hard-delete` |
...
| TJ-036 | A tripwire for the legacy certificate's silent renewal failure | DONE — executor commits `be29eca`, `76357a2`, `15b5280`, merged to `master` as `a7a797a` with `--no-ff`; planner re-ran all four hosts | `ops/cert-tripwire` |
...
| TJ-043c | Doctor: same treatment, plus the 1200px shell cap | DONE — merged to `master` 2026-09-15 with `--no-ff`; **runtime review owed** | `feat/schedule-full-width-doctor` |
...
| TJ-047b | Admin: edit an already-booked reservation | DONE — merged as `b4ad040`; **runtime VERIFIED live 2026-09-15** | `feat/admin-edit-reservation` |
| TJ-047c | Secretary: edit an already-booked reservation | DONE — merged as `00df38a`; **runtime VERIFIED live 2026-09-15**, clinical boundary held | `feat/secretary-edit-reservation` |
| TJ-048 | Open the patient's file from a click on their reservation card | DONE — merged as `04f4325`; **runtime VERIFIED live 2026-09-15**, portal-bubbling regression cleared | `feat/card-opens-patient-file` |
...
- **Why:** TJ-009 §1. There was no model for employee documents; `User` carried only `pictureUrl`. The storage half already worked — `POST /api/upload` passes non-images through untouched and keeps the extension — so what was missing was a table and a UI.

**Original planning pass:** 2026-08-15 — read `prisma/schema.prisma` in full, `src/app/api/upload/route.ts`, and the `PatientFile` model that is the obvious template (`schema.prisma:112-122`), which is what `EmployeeFile` was modelled on.

**The blocker, and how it was cleared.** This task sat BLOCKED while every other TJ-009 split was planned under a stated assumption, because the others cost a string or a re-edit if the assumption was wrong and this one cost a database: there is no `prisma/migrations/`, so the change reaches production only through `prisma db push`, applied directly with no down migration. The user authorised it on 2026-08-15 **conditional on no conflicts or errors**, and that condition was discharged by the two checks recorded below rather than assumed.

**Still true after the push, and it now has three feeders:** nothing in this app ever deletes an uploaded object and the client holds only the Supabase **anon** key, so every upload is permanently orphaned when replaced or when its row is removed. Employee documents add to that leak rather than creating it — see the note at the bottom of this file. **Worth filing the leak as its own task before this ships.**
...
**The cost, recorded so it is not a surprise later:** that modal will then carry name, contact, hours, colour, a password change, an admin re-auth field **and** a file list. It becomes the most crowded surface in the admin area. The next thing that needs to go on it is the signal to lift the whole lot out to (b) rather than squeeze again.
...
**Planner verification: 2026-08-17 — passed.** Read the diff: **1 file, 6 insertions, 0 deletions** — the two ignore globs and their comments, nothing else. No rule, no severity, no source file. Re-ran both checks myself: `npm run lint` → **exactly 55 problems (41 errors, 14 warnings)**, matching the pass's per-tree arithmetic, and the corrected ignore assertion → exit **0**, silent. The 55 that must survive did survive; the count went down by exactly the 22 the pass attributed to the two non-app trees.

**Visual review: not applicable, and stated rather than skipped silently.** `eslint.config.mjs` is not consumed by the build or by the running app — it has no runtime surface. `npm run build` exit 0 is the only thing worth asserting about it, and it holds.

**`npm run lint` is now a meaningful command for the first time in this project.** Every task in this queue up to now has carried the instruction *"do not run `npm run lint`, `master` fails it"*, forcing each one to hand-list an `npx eslint` invocation over its own Scope. That workaround can stop. The 55 remaining are real and worth their own task or tasks — 37 `react-hooks/set-state-in-effect` across every admin list page, 4 React Compiler memoization, 10 `<img>`/LCP, 5 hook dependencies — but **making the command honest was the precondition for ever treating it as a gate**, and that is all this task set out to do.
...
- `npx eslint --no-warn-ignored src/generated/prisma/browser.ts design_handoff_landing_and_blog_cms/support.js` exits **0** with no output — the ignore applies even when the path is named explicitly. **Corrected 2026-08-17.** This step originally asked for `npx eslint "src/generated/**/*.ts"` to report `0 problems`, **an output shape ESLint 9's flat-config CLI cannot produce**: for an entirely-ignored target it either hard-errors on the glob (exit **2**, `Oops! ... all of the files matching the glob pattern ... are ignored`) or emits an explicit `File ignored because of a matching ignore pattern` warning, which counts as 1 problem. `--no-warn-ignored` suppresses that notice and is the only form that yields a silent 0. The executor hit both variants, characterised each, and reported instead of accepting "close enough".
...
**Planning pass:** 2026-08-18 — `src/lib/auth.config.ts`, `src/middleware.ts`, `next.config.mjs` and `.gitignore` read end to end.

- Confirmed `publicRoutes` sits at `auth.config.ts:26` and reads exactly `const publicRoutes = ["/", "/api/auth", "/blog", "/api/public"];`, indented **12 spaces**.
- Confirmed the membership test is `pathname === route || pathname.startsWith(route + "/")`, so a single `"/clinic"` entry covers both `/clinic` and everything beneath it. No second entry is needed.
- Confirmed the middleware matcher in `src/middleware.ts` already excludes `.css`/`.js`/image extensions, so legacy asset requests like `/clinic/vendors/bundle.css` bypass auth today regardless of this change.
- Confirmed **no route named `clinic` exists** anywhere under `src/app/` — so this exposes nothing. It converts a redirect into a 404, and cannot widen the authenticated surface.
...
**Do not touch:** the `/login` redirect branch below it, the three role-based `Response.redirect` blocks, the `pages`/`session` config, `src/middleware.ts` (its matcher is correct as it stands), or `next.config.mjs`. Do **not** add a `/clinic` route, page, or rewrite to this app — the proxy owns that path.

**Instructions**

**Step 1 — `src/lib/auth.config.ts`: widen `publicRoutes`.**
...
- Start the built app on a free port (`npx next start -p 3100`) and check, with no session cookie:
  - `curl -s -o /dev/null -w "%{http_code}\n" http://localhost:3100/clinic` → **404**, not `307`.
...
### TJ-018 — Document the production environment variables

- **Status:** DONE — task commit `8bb70df` on `docs/env-example`, merged to `master` as `cd3d4cf` with `--no-ff`. Planner-verified from the diff on 2026-08-21; documentation-only, so no visual review (per the protocol, such tasks go straight from REVIEW to DONE).
- **Branch:** `docs/env-example`
- **Why:** `.gitignore` ignores `.env*` and **no `.env.example` exists**, so nothing in the repo records which variables a deployment needs. `Production_Cutover.md` Phase 1 requires setting all seven on the hosting platform; without a checked-in list the person deploying is reading someone's live `.env` over their shoulder or guessing. Two of the seven carry production-specific guidance that is easy to get wrong and expensive to diagnose: `DATABASE_URL` must be Supabase's **pooled** connection string because `src/lib/prisma.ts` opens a `pg.Pool` per instance — the direct string exhausts Postgres connection slots on a serverless host and produces intermittent 500s that look like application bugs — and `AUTH_URL` must track whichever hostname is currently being served, a stale value being the standard cause of a login that loops forever.

**Planning pass:** 2026-08-18 — `.env` read **for key names only** (`grep -oE '^[A-Za-z_][A-Za-z0-9_]*='`; no values were read into context or into this file), plus `.gitignore`, `prisma.config.ts`, `src/lib/prisma.ts`, `src/lib/auth.config.ts` and `package.json`.

- Confirmed exactly seven variables: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `DATABASE_URL`, `AUTH_SECRET`, `AUTH_URL`, `GOOGLE_PLACES_API_KEY`, `GOOGLE_PLACES_PLACE_ID`.
- Confirmed `AUTH_SECRET` and `AUTH_URL` appear **nowhere in `src/`** — NextAuth v5 reads them from the environment itself. They are deployment config, not code, which is exactly why they need documenting.
- Confirmed `DATABASE_URL` is consumed twice: `prisma.config.ts` (CLI) and `src/lib/prisma.ts`, the latter via `new pg.Pool({ connectionString })`. The pooling note is grounded in that line, not received wisdom.
- **Corrected a trap:** `.gitignore:34` is `.env*`, which would ignore `.env.example` too. A negation is required and must come *after* the pattern it negates, or the file silently never gets committed. This is the part of the task most likely to be done wrong.
- Confirmed `src/generated/prisma/` is gitignored and rebuilt by the `postinstall` script, so it needs no env entry.
- Both files are **CRLF**.

**Planner verification:** 2026-08-21 — read `git show cd3d4cf` in full rather than trusting the report. Two files, **26 insertions, 0 deletions**: `.env.example` (new, 25 lines) and the single `!.env.example` line in `.gitignore`. Nothing outside Scope was touched and `.env` appears nowhere in the diff. `npm run build` green, exit 0.

Measured before/after: the repo went from documenting **none** of the seven deployment variables to documenting **all seven**. `.env.example` is tracked (`git ls-files --error-unmatch .env.example`); `.env` is still ignored (`git check-ignore -v .env` → `.gitignore:34:.env*`) and still untracked. The example's key set was diffed against the real `.env` **by name only** — no value was read into context — and the two sets match exactly. Six keys ship empty; `AUTH_URL` carries only `http://localhost:3000`, which is a default, not a secret.

**One Verification bullet was a prediction, and it was wrong — but not in the way it first appears.** The bullet expects `git check-ignore -v .env.example` to report *no match* and exit non-zero. What that command actually does depends on whether the file is tracked yet, which the bullet never considered:

| State | Command | Output | Exit |
|---|---|---|---|
| Untracked — i.e. when the executor ran it | `git check-ignore -v .env.example` | `.gitignore:35:!.env.example` | **0** |
| Tracked — i.e. today, post-merge | `git check-ignore -v .env.example` | *(none)* | **1** |
| Tracked, with `--no-index` | `git check-ignore -v --no-index .env.example` | `.gitignore:35:!.env.example` | **0** |

At the moment the check was run the prediction was wrong on both counts: **`-v` prints the matching pattern even when the match is a negation**, and exits 0. Today the same command *does* return no output and exit 1 — which looks like the prediction quietly coming true, but the mechanism is unrelated: **git exempts paths tracked in the index from the ignore check altogether**, so no pattern is consulted at all. `--no-index` restores the negation line and exit 0, which is the proof. The claim the bullet stood for — the example is committable — is correct, and `git ls-files` establishes it directly without depending on any of this. **Rule for future passes: `check-ignore` against a file the task itself is about to commit changes its answer mid-task. Assert tracking with `git ls-files`, not un-ignoring with `check-ignore`** — and note this is the fourth time a bullet predicting a tool's *rendering* of a result has been wrong (TJ-009f2, TJ-015, TJ-010a, now here), exactly as the Findings archive already warns.

**A splice in this file, repaired in the same commit.** The last Verification bullet below was cut in two by an 86-line block pasted into the middle of it: the line ended at ``- `grep -c '=`` and the sentence resumed 86 lines later with `` .env.example` shows the placeholder keys are empty``. The pasted block was an **exact stale duplicate** of the `## Notes for the planner` Findings archive — byte-identical to the canonical copy apart from one finding added to that copy since. It was introduced by `5cf030f`, the same commit that wrote this task, so it predates the executor's pass and nothing in TJ-018's diff caused it. Deleted behind a pre-flight gate that re-compared both copies at deletion time rather than trusting an earlier reading; the canonical archive is untouched and the intro line now appears exactly once. The bullet was restored with the command that was actually run (`grep -c '=$'`) and its measured output pasted, per this file's own rule about numeric bullets. **Worth carrying forward: a paste that lands mid-line leaves the task still *looking* complete — `**Done when:**` rendered fine and the Queue row still read READY — while one of its Verification checks had silently ceased to exist.**

**Scope — touch only these:**
- `.env.example` (new file)
- `.gitignore`

**Do not touch:** `.env` — do not commit it, do not rename it, and **do not copy any value out of it into `.env.example`**. Placeholders only. No other ignore rule in `.gitignore`.

**Instructions**

**Step 1 — `.gitignore`: stop ignoring the example.**
...
**Step 2 — create `.env.example`** with exactly this content and no real values:

```
# Copy to .env for local development, or set these on the hosting platform for
# production. Never commit .env — see Production_Cutover.md for deployment notes.

# --- Supabase -------------------------------------------------------------
# Project URL and anon key. The anon key is public by design.
NEXT_PUBLIC_SUPABASE_URL=
NEXT_PUBLIC_SUPABASE_ANON_KEY=

# --- Database -------------------------------------------------------------
# PRODUCTION: use Supabase's POOLED connection string, not the direct one.
# src/lib/prisma.ts opens a pg.Pool per instance; on a serverless host the
# direct connection exhausts Postgres connection slots under load.
DATABASE_URL=

# --- Auth (NextAuth v5) ---------------------------------------------------
# AUTH_SECRET: generate a fresh value per environment. Never reuse dev in prod.
AUTH_SECRET=
# AUTH_URL: must match the hostname actually being served, including scheme.
# A stale value here is the usual cause of a login that loops forever.
AUTH_URL=http://localhost:3000

# --- Google Places (reviews section) --------------------------------------
# Optional. Absent, the reviews section degrades; nothing crashes.
...
**Verification:**
- `npm run build` passes.
- `git check-ignore -v .env` still reports a match — the real file stays ignored.
- `git check-ignore -v .env.example` reports **no match** and exits non-zero — the example is committable.
- `git status --short` lists `.env.example` and `.gitignore`, and **does not list `.env`**.
- `grep -c '=$' .env.example` returns **6** — the six keys that ship empty. `AUTH_URL` is the seventh and carries only the `http://localhost:3000` default, so it is deliberately not counted. No value leaked.

**Done when:**
- [x] `.env.example` is tracked, `.env` is still ignored
- [x] All seven variables present, every one without a real value
- [x] Build passes

---

### TJ-019 — Migrate the `middleware` file convention to `proxy`

- **Status:** BACKLOG — no planning pass. **Must not be executed during the production cutover window.**
...
- **Status:** DONE — executor commits `be29eca`, `76357a2`, `15b5280` on `ops/cert-tripwire`, merged to `master` as `a7a797a` with `--no-ff`. Planner-verified 2026-08-24 by re-running all four verification hosts independently rather than accepting the report; every figure reproduced, including the 59-day remaining count and the baseline MD5 before/after a refused `--update`. **Two defects were caught in review — see the entry in *Notes for the planner*.** Tooling-only, no `src/` change, so no visual review.
...
**A defect in this task's own spec, found at review and fixed on the branch before merge.** The Instructions specified `z-index: 1200` verbatim while the Regression-risk paragraph asserted the popover "sits above the calendar's internal `z-index: 3` sticky labels and below the `z-index: 1000` modal overlay". **1200 is above 1000, not below** — the literal contradicted the invariant it was meant to satisfy. The executor implemented the literal exactly as instructed and **reported the contradiction instead of silently correcting it**, which is precisely the behaviour the planner/executor split exists to produce. Auditing every `z-index` actually in play settled the right value: `Calendar.tsx:286` sticky label **3**, `ReservationSlot.tsx:328` `.menu-panel` **200**, `admin/page.tsx:510` `.modal-overlay` **1000**. **200** is this codebase's established layer for a body-portaled floating panel — the very component whose portal pattern this popover was told to reuse. The spec borrowed the pattern and then invented a new layer for it. Fixed to 200 in `e436b96`, with a comment recording why, so nobody raises it again on the assumption that bigger is better.

**What is still owed, and why I did not merge.** Every remaining Done-when box needs the admin dashboard on screen, which is behind a login; entering credentials is not something I do. The 8-column claim in particular is **my own measurement and must be proven, not assumed** — the pass derived it from a layout probe, not from the real authenticated page. Note that `localhost:3000` runs against the live Supabase database, so this review is read-only by nature (layout only, no writes) and is safe to perform whenever a person signs in. Same blocker as TJ-040 and TJ-010a.
...
- **Status:** DONE — task commit `b9d028e`, merged to `master` as `7fa4e89` with `--no-ff` on 2026-09-15. Verified with every command re-run. **Runtime review owed** — see the last paragraph; it is not the same deferral TJ-043a and TJ-043b took.

**Planner verification, 2026-09-15 — re-run, not accepted.** Diff read in full: exactly the two Scope files, **15 insertions, 11 deletions**, `tasks.md` not among them, branch cut from `cc64619`. All six instruction steps landed anchor-for-anchor with nothing extra — the popover swap, the count moved to its own span, the sidebar deleted, the four CSS rules replaced, the `768px` block gone outright, and the shell cap at `1400px; margin: 0 auto`. Commands were re-run by the planner in the **main worktree**, which has the real `.env` and a generated Prisma client, rather than taken from the executor's report: `npx tsc --noEmit --incremental false` exits **0** both before and after the merge; `npm run build` exits **0** after it; `git diff --check` clean. Lint was measured on both sides rather than assumed: `master`'s baseline for the two files is **3** `react-hooks/set-state-in-effect` errors at `49:37`, `68:23` and `69:23`, and after the merge it is the identical 3 at the identical positions — **zero new**. All three sit in effects near the top of the file, nowhere near the edits at 137–203. All four assertion greps hold: no `sidebar-col`, no `DatePickerCalendar`, no `current-date`, no `768px`. The live Duplicate modal is intact — 8 references to its state and handlers survive, and no `dupe` line appears in the diff.

**Two corrections to the executor's report, recorded so nobody trusts them later.** (1) It reported that no `.env` exists in the main worktree and created a placeholder one in its own worktree to get past `supabase.ts` throwing at module load. **A real `.env` does exist in the main worktree.** The placeholder was unnecessary; it was harmless — gitignored, fake values, never committed, confirmed absent from `b9d028e` — but its `npm run build` therefore proved less than the planner's re-run, which used the real file. (2) Its literal `git diff --stat master..HEAD` read **10 files**; that was `master` moving underneath it, not scope creep. `git merge-base` is `cc64619` and `git diff cc64619..master -- src/app/doctor/` is **empty**, so the fork point was clean and nothing contaminated the diff.

**The concurrency call in the pass held.** A parallel agent merged TJ-048 (`04f4325`) and TJ-047a (`1a7157b`) into `master` while this task was being executed. Neither touched `src/app/doctor/*`, so this branch merged with no conflict, and both of those tasks' "`doctor/page.tsx` is unedited" assertions were branch-scoped and unaffected — as the pass predicted.

**Runtime review owed — do not read this as TJ-043a/b's deferral.** Those two were deferred to the live site by user decision; this one has simply not been run yet. Three items are unproven: the popover's click / outside-click / Escape / scroll behaviour on this page, the **8 columns at 1440px** claim (measured for the secretary shell and carried here by structural equivalence, never observed on the doctor page), and the popover-over-`.modal-overlay` case — **this surface is the first place that is testable**, because the Duplicate modal here is live code rather than the dead modal the other two dashboards carry.
...
- **Status:** DONE — task commit `0db6c6d` (amended from `963584f`), merged to `master` as `1a7157b` with `--no-ff` on 2026-09-15. **Live `PUT`/`POST` checks waived by user decision; owed against the live site.**

**Planner verification, 2026-09-15 — re-run, not accepted.** Read the full `git diff 5bfeb99..feat/clinic-window-helper`: exactly the three Scope files, 97 insertions and 24 deletions, `src/lib/clinicHours.ts` created. `duplicate/route.ts` untouched, as instructed — TJ-046 keeps it. Confirmed the `POST` replacement preserves **both** error strings verbatim, so no caller's wording moved. `grep -rn "CLINIC_" src/` returns **four hits, all inside `src/lib/clinicHours.ts`** — the rule is stated in exactly one place. `grep -rn "2000-01-01" src/` returns **one hit, inside a comment** explaining the bug that was removed; the fallback itself is gone. Gates re-run by the planner on the merged `master`, not on the branch: `npx tsc --noEmit --incremental false` exits **0**, `npm run build` exits **0** (`✓ Compiled successfully`). The 8 boundary cases were **re-run by the planner against the real compiled helper** — `tsc`-emitted from `src/lib/clinicHours.ts` and imported, not retyped — and all **8/8** match TJ-042a's proven boundaries, including the `18:00` one-hour case that only passes because the comparison is `>` and not `>=`.

**The spec was wrong and the executor caught it.** As filed, both call sites read `if (!windowCheck.ok)`, which **does not compile in this repo**: `tsconfig.json` sets `"strict": false`, and without `strictNullChecks` TypeScript will not narrow a discriminated union through truthiness on its discriminant. The executor implemented the instruction verbatim, hit the failure on `tsc` **and** `next build`, built an isolated repro outside the repo to prove it was a language rule rather than a local misconfiguration, and stopped to ask rather than substituting a fix of its own. The planner re-ran both forms with this repo's own TypeScript 5.9.3 before issuing the correction — `!ok` passes under `"strict": true` and fails under `"strict": false`; `=== false` passes under both. Both call sites now carry a comment saying why, so it does not get "tidied" back. The general lesson is recorded under "Notes for the planner".

**Live checks: not performed — user decision, 2026-09-15:** *"no need for runtime verification, we push to master then we check on the live website."* Bullets (a)–(d) on `PUT` and the `POST` 03:00/18:00 re-check were **not run**. The same blockers applied as for TJ-048 — no agent can sign in, and the only database is the clinic's production one. The executor's inspection argument that `POST` cannot have changed (it now calls the identical `checkClinicWindow` proven by the 8 cases above) is sound and is recorded **as inspection, not as a passed live test**. What to check first on the live site: that an ordinary in-window reschedule still saves, since that is the path every later edit screen depends on.
...
**RUNTIME VERIFIED ON THE LIVE SITE, 2026-09-15 — read-only, nothing was saved.** The edit screen for reservation 57 was opened and its fields compared against the same record fetched from `GET /api/reservations`. **The date round-trip, which is the one silent failure this task could ship, is correct:** stored `sessionDate` `2026-09-15T00:00:00.000Z` renders as **2026-09-15** with no day shift, and stored `sessionTime` `07:00Z` renders as **10:00**, matching the card that was clicked. Doctor, payment and the two-hour checkbox all match the stored record; `min="07:00"` / `max="18:00"` are on the time input; the patient shows read-only with a working file link; the Next Session Note field is present. **Not exercised:** the save round trip and the `03:00` refusal path, both of which write to the clinic's live database — they remain owed. Superseded waiver: the admin edit round trip, the `03:00` refusal path, the doctor and secretary menu checks and the 320px/2560px stacking check were **not run and are not claimed**. On the live site, check the edit screen's **Date and Time fields against the card you clicked** before anything else: those two lines are the only place in this task where a wrong answer is silent, persists to the database, and looks correct on screen.
...
- **Status:** DONE — task commit `8cbd5c1`, merged to `master` as `00df38a` with `--no-ff` on 2026-09-15. **Runtime review waived by user decision; owed against the live site.** With this, TJ-047 is complete: admin and secretary can both edit a booked session.

**Planner verification, 2026-09-15 — re-run, not accepted.** Read `git diff master..feat/secretary-edit-reservation`: exactly the two Scope paths, **247 insertions and 0 deletions**. `git diff -- src/app/components/` and `git diff -- src/app/admin/` each return **0 lines** — the shared components and the admin half were not touched, which matters because another executor was editing `Calendar.tsx` and `ReservationSlot.tsx` at the same moment. Gates re-run: `tsc` **0**, `npm run build` **0**, with `ƒ /secretary/reservations/[id]/edit` in the route table beside the admin one.

**The clinical boundary was checked against the branch by the planner, not taken from the report.** `grep -c "nextSessionNote"` on the new page returns **0**; `grep -cE "soapNote|subjective|objective|assessment|treatmentPlan|diagnosis"` returns **0**. A secretary's edit screen carries the session note and nothing clinical, matching what `canAccessClinical` already refuses them at the API. Every class the new markup uses was also cross-checked against the rules the new file defines — **none missing**, and the four `.patient-readonly` / `.pr-*` rules TJ-047b introduced are present in this file's own style block, which is the copy hazard this task was warned about.

**RUNTIME VERIFIED ON THE LIVE SITE, 2026-09-15 — read-only.** `/secretary/reservations/57/edit` loads with the same correct date (**2026-09-15**) and time (**10:00**) as the admin page, and **the clinical boundary holds**: the page has **one** textarea against the admin page's **two**, "Next Session Note" does not appear anywhere in its text, and a scan for SOAP / Subjective / Objective / Assessment / Diagnosis / Treatment Plan returns nothing. The secretary dashboard's card menu shows **Edit** and **no Session Details**. **Not exercised:** the save round trip, which writes to live data. One incidental observation, pre-existing and not caused by this task: an ADMIN can open the `/secretary` routes, so the check above was run from an admin session.
...
- **Status:** DONE — task commit `ca9d41a`, merged to `master` as `04f4325` with `--no-ff` on 2026-09-15. **Runtime review waived by user decision; the behaviour is owed against the live site.**

**Planner verification, 2026-09-15 — re-run, not accepted.** Read the full `git diff 5bfeb99..feat/card-opens-patient-file` rather than the executor's report: exactly the four Scope files, 36 insertions and 2 deletions. `src/app/doctor/page.tsx` has a **zero-line diff** — the proof that the new prop is properly optional, and the check this task was most likely to fail. `tasks.md` untouched. `grep -c "stopPropagation"` against the branch's `ReservationSlot.tsx` returns **2**, so both load-bearing calls survive. All 14 Instruction steps applied verbatim; no anchor mismatches, reported or visible in the diff. Both gates re-run by the planner **in a clean worktree rather than the main checkout** (see the nested-worktree note under "Notes for the planner" — the main checkout's measurements are no longer trustworthy): `npx tsc --noEmit --incremental false` exits **0**, `npm run build` exits **0**.

**RUNTIME VERIFIED ON THE LIVE SITE, 2026-09-15** — after the waiver, the user signed in and the planner drove their browser against `therapyjo.com`, **read-only: no reservation was created, saved, checked in or duplicated.** Day under test: 2026-09-15, 41 reservations. **(1) Card body click** — clicked the card for a mid-day patient and landed on `/admin/patients/42` whose heading is that same patient, so the reservation-id → patient-id resolution is right and is not off by one. **(2) The admin menu** reads Session Details · Check In · Waiting · Edit · Duplicate · Cancel · Delete — both new items present, Edit directly above Duplicate. **(3) The secretary menu** is the same minus **Session Details**, confirming the optional prop is passed only by admin. **(4) Session Details** opens the moved modal (Phone / Doctor / Status / Payment) with the URL unchanged. **(5) The portal-bubbling regression is cleared:** choosing **Duplicate** opened its dialog and `location.pathname` stayed `/admin` — no navigation to the patient file and no jump to the new-reservation form. That was the one failure this task could have shipped silently, and it does not occur.

**A false alarm worth recording, because it nearly became a bug report.** The first card-click test appeared to open the *wrong* patient. It was the planner's error, not the app's: the screenshot coordinate frame is **1568×698** while the page viewport is **1920×855**, so clicks driven from screenshot coordinates land about 22% off and hit a different card. Re-run by clicking the **element reference** rather than a coordinate, the behaviour was correct both times. **Rule: when driving a browser, click by element ref; if you must use coordinates, check `innerWidth`/`innerHeight` against the screenshot frame first.**

**Original waiver, superseded by the above — user decision, 2026-09-15:** *"no need for runtime verification, we push to master then we check on the live website."* The eight runtime bullets below were **not run and are not claimed** — card-click navigation on either dashboard, the Session Details item, the portal-bubbling regression (Duplicate / Check In must not navigate), empty-grid-space navigation, the doctor dashboard's unchanged menu, and the 320px check. The blocker was real and independent of the decision: `src/lib/auth.ts` uses a credentials provider with **no dev bypass**, `prisma/seed.ts` records that the old default password was deliberately removed because this login is publicly reachable on a live medical clinic, and the only database is that clinic's production Supabase — so no agent can sign in, and the portal-bubbling check would have written to real clinic data. **What is proven here is the code. What is owed is the behaviour**, and the portal-bubbling case is the one to look at first on the live site: if choosing *Duplicate* or *Check In* also jumps to the patient file, that is this task's regression and nothing else's.
...
- **Status:** DONE — task commit `cd83e7a` (amended from `e252a3e`), merged to `master` as `f762343` with `--no-ff` on 2026-09-15. **The task as filed contained two CSS bugs; both were found at review, corrected, and proven in a browser.** Runtime density counts remain waived and owed against the live site.

**Planner verification, 2026-09-15.** `git diff 4604ced..feat/narrower-schedule-columns`: exactly the two Scope files, 48 insertions and 10 deletions. `MIN_COL_WIDTH` is **110** on `master`. `ROW_HEIGHT`, `MAX_COL_WIDTH`, `COL_GAP`, `LABEL_WIDTH`, the packing engine and both `@media` blocks are untouched. Gates: `tsc` **0**, `npm run build` **0**.

**Two bugs in the filed spec, neither of which any gate could see.** The executor implemented the task verbatim and everything passed; the defects only surfaced because the review asked for the finished `<style jsx>` block and read the cascade. They were then **measured in a browser** — a served test page mirroring the real rule order, with computed styles read at a 130px container — rather than argued about:

1. **An element cannot be styled by a container query it establishes itself.** The task put `container-type: inline-size` on `.slot` and then wrote `@container { .slot { padding } }`. `.slot` is outside its own container, so both `.slot { padding }` tiers were **dead**, and no reordering could have revived them. Measured: `.slot` padding-left stayed at the base `0.65rem` (10.4px).
2. **A container query adds no specificity, so a base rule declared later wins.** `.slot-right` and `.menu-trigger` are descendants and did match, but their base rules sit **below** the `@container` blocks, so they won on source order. Measured: `.slot-right` margin-left stayed 4.8px and `.menu-trigger` font-size stayed 19.2px. `.slot-name` worked only by the accident of being declared above the tiers.

Between them these killed exactly the two things the 110px floor depends on — reclaiming card padding and shrinking the `⋮` trigger — while leaving the font tiers and the phone/separator hiding working, so the change looked broadly right.

**The fix, and its proof.** The query container moved up to `.card-wrap` in `Calendar.tsx` — the box that *is* one column wide — and was **named** (`container: reservation-card / inline-size`) so the rules in `ReservationSlot.tsx` bind to it explicitly across the two style blocks. The three tiers moved to the **end** of `ReservationSlot`'s style block, after `.menu-item.danger:hover`, so nothing can outrank them on source order. Re-measured in the browser against the corrected structure at a 130px container: **all ten declarations now apply** — `.slot` padding 5.6px, `.slot-name` 11.52px, `.slot-phone` and `.slot-sep` `none`, `.slot-note` `none`, `.slot-right` margin 2.4px, `.menu-trigger` 16px / 2.4px — and `.slot-time` stays visible. The 150px tier correctly outranks the 200px tier where both match.

**RUNTIME VERIFIED ON THE LIVE SITE, 2026-09-15 — read-only.** Measured on the real 2026-09-15 day (41 reservations) at a pane of **1334px** — within 2px of the 1336px this task predicted. The day packs **10 columns**: `--pane-min-width` is **1226px** and there is **no horizontal scrollbar** (`scrollWidth === clientWidth === 1334`). **On the old 150px floor the same day would have needed 1626px and would have scrolled**, so the change is doing exactly what it was filed to do, on real data. **Both tiers were confirmed firing on real cards:** the narrowest column is **121px** and its card reports padding-left **5.6px** and trigger font **16px** — the two declarations that were dead before the review fixed them — with phone and separator `display: none`, the name **not truncated**, and the time still visible; the widest card (320px) keeps base padding 10.4px, 12.8px name and full phone + time. Superseded waiver: the density counts (9 → 10 columns on a ~1440px window) were not measured at merge time. On the live site, open a busy day and confirm the schedule fits more columns than it did before scrolling sideways, and that a card at the narrowest width still shows the patient's **name and time** — the phone and the note are expected to be gone by design.
...
**RUNTIME VERIFIED ON THE LIVE SITE, 2026-09-15 — read-only.** The real day needs 1226px in a 1334px pane, so scaling is correctly **not** engaged there (`--row-height` 68px, `--label-width` 72px, factor exactly 1). To exercise it, the pane was narrowed **in the DOM only** — no data touched, no window resized, and the original width restored immediately afterwards. Results: at a **834px** pane the factor hit the **0.72 floor** (`--row-height` **49px** = round(68×0.72), `--label-width` **52px**, `--pane-min-width` **883px**) and then scrolled, which is the designed "scale to the floor, then scroll" behaviour; at a **634px** pane the factor returned to exactly **1**, confirming the `MIN_SCALABLE_PANE_WIDTH` pin and the conservative pane-versus-viewport choice recorded above. Restoring the width returned every value to its original. **Alignment, the thing scaling was most likely to break, was checked across every card on the day:** all row heights are 68px and every off-hour card's `top` equals `round(minute/60 × rowHeight)` exactly — 2:24 PM → 27px, 5:20 PM → 23px, 5:25 PM → 28px, 5:30 PM → 34px. **Still unproven:** no **two-hour** session exists in that day's data, so "spans exactly two rows" has not been observed on real data — check it the next time one is booked. Superseded waiver: open the busiest day available and confirm it fits more columns than it used to before scrolling sideways; then check the two things scaling is most likely to break — that a **two-hour** session still spans exactly two rows and a **half-hour** start still sits halfway down its row, and that the `⋮` menu still opens **against its own card** rather than offset, which is what the no-transform decision was made to protect.
...
### TJ-050 — Nested agent worktrees corrupt every whole-project measurement

- **Status:** BACKLOG — no planning pass. Do not execute against this ID.
- **Why:** `tsconfig.json` includes `"**/*.ts"` and `"**/*.tsx"` and excludes only `node_modules`; ESLint walks the same tree. When an agent worktree is created **inside** the repo at `.claude/worktrees/<agent>/`, every tool that measures the whole project sees two or three complete copies of `src/`. Measured 2026-09-15: `npm run lint` reported **63 problems (48 errors, 15 warnings)** on a clean `master` before any worktree existed, then **171** and then **237** on *identical, unmodified* code once two worktrees were present — 196 MB of them. `tsc` and `next build` read the same glob and are affected too, but silently: the extra copies usually compile clean, so the only symptom is a slower run.
...
- **Status:** DONE — task commit `8ad6a7b`, merged to `master` as `138c3d0` with `--no-ff` on 2026-09-17; pushed by the user the same day (the agent's own `git push` was refused by the auto-mode classifier as *Out-of-Place Publication*). **Runtime VERIFIED live 2026-09-17, signed out:** `/Login` and `/LOGIN` both answer `302 Location: /login` — note the **`callbackUrl` is gone**, which is the observable signature of the new guard firing ahead of the auth branch rather than after it — and both chains terminate at `200` in a single hop, so the loop the first conjunct guards against does not occur. `www.therapyjo.com/Login` reaches the same place in two hops. **Regression surface re-measured and intact:** `/login` still `200` with `X-Matched-Path: /login`; `/admin` signed out still `302 → /login?callbackUrl=%2Fadmin`, so callbackUrl is still minted for genuinely protected routes; `/Login.aspx` still `307 → /clinic/Login.aspx`; `/clinic/` still `200` with `X-Powered-By: ASP.NET`; `/clinic` still `307 → /clinic/`. **One bullet remains unobserved rather than passed:** the signed-in `/Login` path, because no agent can hold a clinic session. It is the same guard on the same line, firing before the auth branch, so it redirects to `/login` and the pre-existing `pathname === "/login" && isLoggedIn` rule forwards to the role dashboard — reasoned, not measured, and recorded as such.
...
- **A waived runtime review is recoverable, and most of it does not need write access.** TJ-047b/c, TJ-048, TJ-049a and TJ-049b all merged with their runtime bullets waived because no agent can sign in. When the user signed in themselves on 2026-09-15 and handed over the browser, **almost every bullet turned out to be answerable read-only** — navigation, menu contents, form field values, computed styles, card geometry, and the presence or absence of a clinical field. The only genuinely unavailable checks were the ones that *write*: saving an edit, the out-of-window refusal path, and a status transition. **Two habits follow.** When writing a task's Verification block, **separate the read-only bullets from the write bullets explicitly** — the read-only ones can be honoured later without touching the clinic's database, and a block that mixes them reads as entirely blocked when it is mostly not. And **prefer an assertion over an eye**: the strongest evidence in this pass was not a screenshot but `getComputedStyle`, `scrollWidth === clientWidth`, and a form's `input.value` compared against the same record fetched from the API. A screenshot would have flattered every one of those. (Recorded after the live verification pass, 2026-09-15.)
- **Driving a browser by screenshot coordinates is unsafe when the screenshot is scaled, and the failure looks exactly like an application bug.** During the live pass the screenshot frame was **1568×698** while the page viewport was **1920×855** — a 0.816 factor. Clicks issued in screenshot coordinates landed roughly 22% off and hit neighbouring cards, which produced a convincing false positive: a card click appeared to open **the wrong patient's file**. It was very nearly written up as a TJ-048 defect. Clicking by **element reference** instead resolved it immediately and the behaviour was correct. **Rule: click by element ref; if coordinates are unavoidable, read `innerWidth`/`innerHeight` and compare them against the reported screenshot frame before trusting a single click.** A corollary worth keeping: when a UI test "fails" in a way that implies a deep logic bug in code that reads like a simple id lookup, suspect the harness before the code. (Found during the live verification pass, 2026-09-15.)

- **A CSS spec can be wrong in ways that every gate passes, the diff looks right, and the change half-works — and the only way to find out is to measure computed styles.** TJ-049a shipped three `@container` tiers. Two of their declarations could never apply (an element cannot be matched by a container query it establishes itself) and two more lost silently on source order (a container query adds no specificity, so a base rule declared later wins). `tsc`, `next build` and a careful diff read all passed; the executor's work was faithful. **What caught it was asking for the finished `<style jsx>` block and reading the cascade**, then confirming with a served test page and `getComputedStyle` instead of reasoning — a browser answered in one call what an argument would have left uncertain. **Three rules for any future container-query work here.** Put `container-type` on an *ancestor* of everything the tiers style, never on the element they style. **Name the container** when the query lives in a different component's style block from the container — it costs nothing and makes the binding explicit instead of "nearest ancestor that happens to qualify". And **put `@container` blocks last in a styled-jsx block**, because specificity will not save them from a base rule written below. A postscript worth keeping: the diff *looked* correct to a careful reader, which is exactly why "the executor implemented it verbatim" is not evidence that a spec was right. (Found during the TJ-049a review, 2026-09-15.)

- **The lint baseline is recoverable after all — measure it in a clean worktree and it becomes attributable again.** TJ-050 records that nested agent worktrees inflate `npm run lint` unpredictably (63 → 171 → 237 → 586 → 674 on effectively unchanged code). The fix for *measurement*, as opposed to the underlying defect, turns out to be trivial: run lint in a worktree that has no other worktree nested inside it. Measured that way on 2026-09-15: `master` at `4604ced` (TJ-047a, TJ-048 and TJ-047b all merged) reports **65 problems (50 errors, 15 warnings)**, and TJ-047c's branch reports **67 (52 errors, 15 warnings)**. So the true drift from the original 63 is **+2 for TJ-047b and +2 for TJ-047c**, and every one of the four is the same `react-hooks/set-state-in-effect` finding on the `useEffect(() => { fetchX(); }, [fetchX])` idiom that every data-loading page in this repo already uses — including the two the new pages were copied from. **Not a new violation class and not a regression; the cost of adding a page that loads data.** Two things follow: TJ-030's drift tracking should quote numbers measured this way or not at all, and TJ-050 should note that the workaround costs one `git worktree add` while the real fix is still a `tsconfig` exclude. (Measured during the TJ-047c review, 2026-09-15.)
...
- **Executors share this working tree. Only one may run at a time, and the planner must re-check the branch before every commit.** On 2026-08-15 I dispatched TJ-009h and TJ-009f1 together, reasoning that their file lists were disjoint so they could not collide. **That checked the wrong axis.** There is one `.git`, one `HEAD` and one checkout: when the TJ-009h executor ran `git checkout -b bugfix/default-doctor-colour`, *my* working tree moved with it, and a `tasks.md` edit I made moments later was committed onto the executor's branch as `dec121f` instead of onto `master`.

  **The commit also reverted the executor's fix, and the mechanism is the part worth remembering.** The executor was getting an eslint baseline with `git checkout master -- <file>`, which writes the file **and stages it**. My commit ran `git add tasks.md` — but `git commit` writes the whole index, so it swept up the staged revert. Both halves landed in one commit on the wrong branch: a legitimate planning update and a silent undo of the work being verified. The executor's own `git checkout <branch> -- <file>` then became a no-op, so `git status` looked clean and nothing warned it that it was linting the reverted file.

  **It was caught by the executor, not by me**, because it re-read the diff of a commit that had appeared under it rather than trusting a clean `git status`. Recovery: it preserved the contaminated commit on `backup/tj009h-race-dec121f`, reset its branch to its own commit, and deliberately did **not** try to move my `tasks.md` write onto `master` itself — correctly, since that is the planner's file. I then recovered only the `tasks.md` half from the backup (`f082f04`).

  **Rules this produces.** One executor at a time, full stop — the stacked TJ-009 chain was safe only because it was serial, not because it was well coordinated. If genuine parallelism is ever wanted, each executor needs its own `git worktree`. And before any planner commit: run `git branch --show-current`, and prefer `git commit -- tasks.md` (pathspec) over a bare `git commit`, so a dirty or staged index elsewhere cannot ride along. (Found during TJ-009h / TJ-009f1.)
