...
- **A throwaway `zz-revoke-test` secretary account exists in the production database**, status `RESIGNED`, created 2026-08-15 to prove TJ-007. It cannot log in and it is soft-deleted, so it is harmless; delete it whenever convenient. Same family as the three "test" doctor profiles and the "Test" blog post — pre-launch cruft that should be cleared before handover.
