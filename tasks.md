...
| TJ-005a | Name the content-management capability | VISUAL REVIEW — verified, unmerged; needs admin login to finish | `refactor/content-capability-helper` |
...
- **Status:** VISUAL REVIEW — commit `2205320` on `refactor/content-capability-helper`, **unmerged**. Planner verification passed; the visual review is **half done and inconclusive**, blocked on admin credentials. See both blocks below.
...
**Visual review: 2026-08-14 — PARTIAL, and deliberately not completed. The branch stays unmerged.**

Done, against a **production** build on port 3100, with the served-bytes rule applied first: `.next/BUILD_ID` is `Nn7TZWEKgMFDCJCLJfTkf` and that exact string is present in the served HTML, so the bytes under review are this build and not the process already sitting on :3000.

- **Public site is unaffected**, as expected for an admin-only change: `/` renders all **9** sections (hero → contact), **16** images, **0** broken by `complete && naturalWidth === 0`.
- **The middleware chokepoint behaves**: unauthenticated `/admin/blog` redirects to `/login?callbackUrl=%2Fadmin%2Fblog`, and `/api/blog`, `/api/doctor-profiles`, `/api/pending-changes` all redirect rather than erroring — no 500 anywhere. `/api/public/doctors` still **200**.

**What could not be checked, and why it matters more than what could:** that an **ADMIN still gets 200** from those three endpoints, and that the Blog list, Doctors list, and Approvals badge still populate. That is the exact regression this task can cause — a wrong helper turns the whole CMS into a silent 401 — and every check above passes just as well in the broken case, because they all run logged out.

The seeded default (`admin` / `admin123` from `prisma/seed.ts:28`) is **rejected** by the live database — `error=CredentialsSignin` — so the password was changed after seeding. Minting a session locally from `AUTH_SECRET` was refused as indistinguishable from forging a token, correctly, and was not worked around. **Needs the real admin password from the user to finish.**

Two things that will otherwise waste time on the retry:
- `AUTH_URL` is `http://localhost:3000/`, so NextAuth rewrites auth redirects to **:3000** even when the app is served on :3100 — the logout/login round trip lands on whatever else is running there. Start the review server with `AUTH_URL=http://localhost:3100` overridden for the run.
- The clinic database currently holds **zero** `DoctorProfile` rows — `/api/public/doctors` returns `[]` and the landing page correctly shows "Team profiles coming soon." Pre-existing data state, not a regression, and untouched by this branch. It means the `/admin/doctors` list will legitimately be empty on review; judge that page by whether it renders its empty state and returns 200, not by whether it shows rows.
