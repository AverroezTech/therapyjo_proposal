...
export default function SecretaryPatientProfile({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchPatient = useCallback(async () => {
        ...
        if (!res.ok) { router.push("/secretary/patients"); return; }
    ...
    const statusColors: Record<string, string> = {
        SCHEDULED: "#60a5fa", WAITING: "#fbbf24", CHECKED_IN: "#34d399",
        WITH_DOCTOR: "#a78bfa", CHECKED_OUT: "rgba(255,255,255,0.35)",
    };

    if (loading) return <div style={{ color: "rgba(255,255,255,0.5)", padding: "2rem" }}>Loading…</div>;
    ...
    return (
        <div>
            ...
            <div className="profile-header">
                <button className="btn-back" onClick={() => router.push("/secretary/patients")}>← Back</button>
            ...
            {/* Edit Info Modal — name + phones only */}
            {editingInfo && (
                <div className="modal-overlay" onClick={() => setEditingInfo(false)}>
                    <div className="modal-card" onClick={(e) => e.stopPropagation()}>
                        <h2>Update Patient Info</h2>
                        <div className="form-stack">
                            <div className="form-group"><label>Name</label><input value={infoForm.name} onChange={(e) => setInfoForm({ ...infoForm, name: e.target.value })} /></div>
                            <div className="form-group"><label>Phone 1</label><input value={infoForm.phone1} onChange={(e) => setInfoForm({ ...infoForm, phone1: e.target.value })} /></div>
                            <div className="form-group"><label>Phone 2</label><input value={infoForm.phone2} onChange={(e) => setInfoForm({ ...infoForm, phone2: e.target.value })} /></div>
                        </div>
                        <div className="modal-actions">
                            <button className="btn-cancel" onClick={() => setEditingInfo(false)}>Cancel</button>
                            <button className="btn-save" onClick={saveInfo} disabled={savingInfo}>{savingInfo ? "Saving…" : "Save"}</button>
                        </div>
                    </div>
                </div>
            )}

            {/* Tabs — NO clinical intake tab for secretary */}
            <div className="tabs">
                ...
                <div className="section-card">
                    {patient.reservations.length === 0 ? (
                        ...
                        <table className="data-table">
                            <thead><tr><th>Date</th><th>Doctor</th><th>Status</th><th>Payment</th><th>Injury</th></tr></thead>
                            <tbody>
            ...
            <style jsx>{`
                ...
                .btn-back { background: none; border: none; color: rgba(255,255,255,0.45); font-size: 0.85rem; cursor: pointer; margin-bottom: 0.75rem; display: inline-block; font-family: inherit; }
                ...
                .section-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: var(--radius-md, 4px); padding: 1.5rem; }
                .data-table { width: 100%; border-collapse: collapse; }
                .data-table th { text-align: left; padding: 0.7rem 0.85rem; font-size: 0.74rem; text-transform: uppercase; letter-spacing: 0.05em; color: rgba(255,255,255,0.4); border-bottom: 1px solid rgba(255,255,255,0.06); font-weight: 600; }
                .data-table td { padding: 0.65rem 0.85rem; font-size: 0.85rem; border-bottom: 1px solid rgba(255,255,255,0.04); color: rgba(255,255,255,0.8); }
                ...
                .form-group input { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); border-radius: var(--radius-sm, 2px); padding: 0.55rem 0.75rem; color: #fff; font-size: 0.85rem; outline: none; font-family: inherit; }
                ...
                .btn-save { background: var(--primary, #4CAF93); color: #fff; border: none; border-radius: var(--radius-sm, 2px); padding: 0.5rem 1.25rem; font-size: 0.85rem; font-weight: 600; cursor: pointer; font-family: inherit; }
